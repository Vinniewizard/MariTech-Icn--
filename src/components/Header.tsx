...
export default function Header({
  ...
  return (
    <header className={`sticky top-0 z-40 flex h-16 w-full items-center justify-between border-b px-3 sm:px-6 transition-colors duration-200 gap-2 overflow-x-auto ${
      theme === 'dark' ? 'border-slate-800 bg-slate-900/50 backdrop-blur-md text-white' : 'border-slate-200 bg-slate-50 text-slate-900'
    }`}>
      {/* Brand Logo & Core navigation */}
      <div className="flex items-center space-x-2 sm:space-x-8 flex-shrink-0">
        <div className="flex items-center space-x-2 cursor-pointer flex-shrink-0" onClick={() => onSwitchView('trade')}>
          <div className={`w-8 h-8 rounded flex items-center justify-center flex-shrink-0 transition-colors ${theme === 'dark' ? 'bg-teal-500 text-slate-900 shadow-[0_0_15px_rgba(20,184,166,0.4)]' : 'bg-slate-900 text-white'}`}>
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5">
              <path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
            </svg>
          </div>
          <div className="hidden sm:block">
            <span className={`text-base font-bold tracking-tight uppercase ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>
              MARITECH <span className={theme === 'dark' ? 'text-teal-400 font-light' : 'text-slate-500 font-light'}>INC.</span>
            </span>
          </div>
        </div>
        ...
        <nav className="hidden md:flex items-center space-x-6">
          <button
            ...
            className={`text-xs font-semibold uppercase tracking-widest py-5 transition-all scroll-smooth cursor-pointer ${
              activeView === 'trade'
                ? theme === 'dark' ? 'border-b-2 border-teal-400 text-teal-400 font-bold' : 'border-b-2 border-slate-900 text-slate-900 font-bold'
          ...
          <button
            onClick={onOpenGuide}
            className={`text-xs font-semibold uppercase tracking-widest py-5 transition-all cursor-pointer ${
              theme === 'dark' ? 'text-slate-400 hover:text-white' : 'text-slate-500 hover:text-slate-900'
            }`}
          >
      ...
      <div className={`hidden lg:flex items-center space-x-2 rounded-md px-2.5 py-1 text-xs font-mono transition-colors ${
        theme === 'dark' ? 'bg-slate-950/80 text-slate-400 border border-slate-800' : 'bg-gray-50 text-gray-450 border border-gray-100'
      }`}>
        <span className={`h-1.5 w-1.5 rounded-full animate-pulse ${theme === 'dark' ? 'bg-teal-400 shadow-[0_0_8px_rgba(45,212,191,0.8)]' : 'bg-green-500'}`} />
